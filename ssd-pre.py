...
class SSD(object):
    ...
    #---------------------------------------------------#
    #   load all classes and set color of every bounding boxes with different classes
    #---------------------------------------------------#
    def generate(self):
        model_path = os.path.expanduser(self.model_path)
        assert model_path.endswith('.h5'), 'Keras model or weights must be a .h5 file.'
        
        # all the classes in training set plus background
        self.num_classes = len(self.class_names) + 1

        # load trained model
        # and their trained parameters of this model
        ...
        # set different color of different classes of object
    ...
    def detect_image(self, image):
        ...
        # normalize image data and get prediction of this image
        ...
        # decode the prediction that model output
        results = self.bbox_util.detection_out(preds, confidence_threshold=self.confidence)
        
        if len(results[0])<=0:
            return image

        # select high confidence boxes and use this class as object's class
