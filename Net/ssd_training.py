...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        # compute all the loss, but should compute positive samples loss and negative samples loss
        # classes loss
        ...
        # get positive samples loss
        # get the number of positive samples in every image
        num_pos = tf.reduce_sum(y_true[:, :, -8], axis=-1)
        # pos_loc_loss in every image
        pos_loc_loss = tf.reduce_sum(loc_loss * y_true[:, :, -8],
                                     axis=1)
        # pos_conf_loss in every image
        pos_conf_loss = tf.reduce_sum(conf_loss * y_true[:, :, -8],
                                      axis=1)

        # get three times negative samples (data for sam_nega)
        num_neg = tf.minimum(self.neg_pos_ratio * num_pos,
                             num_boxes - num_pos)

        # which one is bigger than 0 in sam_nega
        pos_num_neg_mask = tf.greater(num_neg, 0)
        # get 1
        has_min = tf.to_float(tf.reduce_any(pos_num_neg_mask))
        num_neg = tf.concat( axis=0,values=[num_neg,
                                [(1 - has_min) * self.negatives_for_hard]])
        # get average negative samples in all traning set
        ...
        # get top_k confidence as negative sample
        _, indices = tf.nn.top_k(max_confs * (1 - y_true[:, :, -8]),
                                 k=num_neg_batch)

        # get negative samples' index
...
class Generator(object):
    ...
    # used to process train.txt data to the form in which SSD model's output
    def get_random_data(self, annotation_line, input_shape, random=True, jitter=.1, hue=.1, sat=1.1, val=1.1, proc_img=True):
        ...
        if (box_data[:,:4]>0).any():
            ...
            return image_data, []
    # read train.txt file and transfer them to mbox_loc, mbox_conf and mbox_priorBox
